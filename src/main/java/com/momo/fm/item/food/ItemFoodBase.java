...
import com.momo.fm.util.NBTStrDef.IDLNBTUtil;
...
public class ItemFoodBase extends ItemFood implements IHasModel {
    ...
    protected void onFoodEaten(ItemStack stack, World worldIn, EntityPlayer player)
    {
        ...
        Item itemfood = stack.getItem();

        if (itemfood == ModItems.MONSTER_STEW) {
            ...
            if (player.getRNG().nextFloat() < 0.5F) {
                player.addPotionEffect(new PotionEffect(MobEffects.FIRE_RESISTANCE, 1200));
            }
            if(player.getRNG().nextFloat() < 0.5F){
